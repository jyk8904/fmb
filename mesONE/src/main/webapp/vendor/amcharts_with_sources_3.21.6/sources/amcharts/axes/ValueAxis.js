    ...
    AmCharts.ValueAxis = AmCharts.Class({
        ...
        updateData: function() {
            ...
            if (_this.synchronizationMultiplier && _this.synchronizeWith) {
                ...
                if ((_this.start === 0 && _this.end == _this.data.length - 1 && isNaN(_this.minZoom) && isNaN(_this.maxZoom)) || (isNaN(_this.fullMin) && isNaN(_this.fullMax))) {
                    ...
                    if (_this.type == "date") {
                        ...
                        if (_this.strictMinMax) {
                            if (this.minimumDate) {
                                _this.fullMin = this.minimumDate.getTime();
                            }
                            if (this.maximumDate) {
                                _this.fullMax = this.maximumDate.getTime();
                            }
                        }
                    }
                }
        ...
        draw: function() {
            ...
            if (!isNaN(_this.min) && !isNaN(_this.max) && _this.foundGraphs && _this.min != Infinity && _this.max != -Infinity) {
                ...
                if (_this.logarithmic) {
                    ...
                    if (!isNaN(_this.minimum)) {
                        fullMin = _this.minimum;
                    }

                    if (!isNaN(_this.maximum)) {
                        fullMax = _this.maximum;
                    }

                    var degr = Math.log(fullMax) * Math.LOG10E - Math.log(fullMin) * Math.LOG10E;

                    var minDegrees = Math.log(_this.minReal) / Math.LN10 - Math.log(fullMin) * Math.LOG10E;
                    var maxDegrees = Math.log(_this.max) / Math.LN10 - Math.log(fullMin) * Math.LOG10E;

                    _this.relativeStart = AmCharts.roundTo(minDegrees / degr, 5);
                    _this.relativeEnd = AmCharts.roundTo(maxDegrees / degr, 5);
                } else {
                    _this.relativeStart = AmCharts.roundTo(AmCharts.fitToBounds((_this.min - _this.fullMin) / (_this.fullMax - _this.fullMin), 0, 1), 5);
                    _this.relativeEnd = AmCharts.roundTo(AmCharts.fitToBounds((_this.max - _this.fullMin) / (_this.fullMax - _this.fullMin), 0, 1), 5);
                }
        ...
        zoomToValues: function(startValue, endValue, skipEvent) {
            ...
            _this.relativeStart = AmCharts.roundTo((startValue - fullMin) / (fullMax - fullMin), 9);
            _this.relativeEnd = AmCharts.roundTo((endValue - fullMin) / (fullMax - fullMin), 9);
            ...
            if (_this.logarithmic) {
                ...
                _this.relativeStart = AmCharts.roundTo(minDegrees / degrees, 9);
                _this.relativeEnd = AmCharts.roundTo(maxDegrees / degrees, 9);
            }
