(function() {
    ...
    AmCharts.AmSerialChart = AmCharts.Class({
        ...
        validateData: function(resetZoom) {
            ...
            var wasResizingLeft = false;
            var wasResizingRight = false;
            var chartScrollbar = _this.chartScrollbar;
            if(chartScrollbar){
                if(chartScrollbar.dragging){
                    wasDragging = true;
                    chartScrollbar.handleDragStop();
                }

                if(chartScrollbar.resizingRight){
                    wasResizingRight = true;
                    chartScrollbar.rightDragStop();
                }

                if(chartScrollbar.resizingLeft){
                    wasResizingLeft = true;
                    chartScrollbar.leftDragStop();
                }                
            }
            ...
            if(wasDragging){                
                chartScrollbar.handleDragStart();
            }            
            if(wasResizingRight){                
                chartScrollbar.rightDragStart();
            }            
            if(wasResizingLeft){                
                chartScrollbar.leftDragStart();
            }                                    
        },
        ...
        validateNow(validateData, skipEvents){
            var _this = this;
            if(validateData && _this.zoomOutOnDataUpdate){
                _this.start = NaN;
                _this.startTime = NaN;
                _this.end = NaN;
                _this.endTime = NaN;            
            ...
            AmCharts.AmSerialChart.base.validateNow.call(_this, validateData, skipEvents);                         
        },
        ...
        handleCursorMove: function(event) {
            ...
            if (event.panning) {
                ...
                var ex = AmCharts.fitToBounds(event.x, 0, cursor.width);
                var ey = AmCharts.fitToBounds(event.y, 0, cursor.height);
                categoryAxis.showBalloon(ex, ey, cursor.categoryBalloonDateFormat, event.skip);
