...
angular.module('app').controller('FmbTotalCtrl',['CmmAjaxService','CmmWorkerSrvc','$http','$scope','$window','$q','$timeout','$mdSidenav','$interval','$location','$rootScope',										
	 ...
	 *-----------------------------------------*/
		...
		function splitPlanProgress() {
			...
			quotient = parseInt($scope.planProgressList.length /count); 	// 몫: 한번에 보여줄 데이터 갯수
			remainder = $scope.planProgressList.length % quotient;			// 나머지
		...
		function secondDataCall(){
			try
			{
				...
				promise5 = CmmAjaxService.select("bas/selectDateRunInfo.do");
...
/*			 		self.gauge.run = "0 라 인";
		...
		function alarmDateRunInfo() {
			try
			{
				...
				{
					test2.dataProvider = $scope.dateRunInfoList;
					test2.validateData();
				}
			}
			finally{}
		...
		function standbyDateRunInfo() {
			try
			{
				...
				{
					test3.dataProvider = $scope.dateRunInfoList;
					test3.validateData();
				}
			}
			finally{}
		...
		function pie() {
			try
			{
				...
				{
					$scope.defectPie.dataProvider = $scope.defectChart;
		...
		function MobileAlarmDateRunInfo() {
			try
			{
				...
				{
					test6.dataProvider = $scope.dateRunInfoList
					test6.validateData();
				}
			}
			finally{}
		...
		function MobileStandbyDateRunInfo() {	
			try
			{
				...
				{
					test7.dataProvider = $scope.dateRunInfoList
					test7.validateData();
				}
			}
			finally{}
		...
		function MobilePie() {
			try
			{
				...
				{
					defectPie.dataProvider = $scope.defectChart
